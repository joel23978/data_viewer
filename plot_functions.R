...
set_x_axis <- function(chart_start_date
  ...
  cht_end_date <<- chart_end_date + (chart_end_date - chart_start_date)/10
...
set_default_values <- function(
  ...
  cht_width <<- 8
  cht_height <<- 5 
  cht_axes_font_size <<- 11
  cht_y_axes_unit_size <<- 11
  cht_title_size <<- 8
  cht_label_size <<- 7 
  cht_legend <<- F
...
chart_formatting <- function(
    input_data = cpi_data
    # series_dates = cpi_data$date
    # , series_values = cpi_data$value
    # , series_names = cpi_data$name
){
  set_default_values()
...
set_chart_defaults_plotly <- function(
    input_data = cpi_data
    , cht_y_min = min(input_data$value)
    , cht_y_max = max(input_data$value)
    , cht_y_increment = 2
